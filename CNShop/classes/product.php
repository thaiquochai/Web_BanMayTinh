	...
	*/
	class product
	{
		...
		public function insert_product($date,$files){
			...
			$cuahang = mysqli_real_escape_string($this->db->link, $date['cuahang']);
			...
			if($product_code ='' || $productName == "" || $productQuantity == "" || $category == "" || $brand == "" || $product_desc == "" || $price == "" || $type == "" || $file_name == ""){
				...
				$query = "INSERT INTO tbl_product(productName,product_code,product_remain,productQuantity,catId,brandId,product_desc,price,type,image,id_cuahang) VALUES('$productName','$product_code','$productQuantity','$productQuantity','$category','$brand','$product_desc','$price','$type','$unique_image','$cuahang') ";
		...
		{
			$query = 
			"SELECT * FROM `tbl_product` WHERE productId='$id' ";
		...
		{
			$query = 
			"SELECT tbl_product.*, tbl_category.catName, tbl_brand.brandName, cuahang.tencuahang, cuahang.id as id_cuahang

			 FROM tbl_product INNER JOIN tbl_category ON tbl_product.catId = tbl_category.catId
								INNER JOIN tbl_brand ON tbl_product.brandId = tbl_brand.brandId
								INNER JOIN cuahang ON cuahang.id = tbl_product.id_cuahang
		...
		public function search_product($tensanpham){
			$query = "SELECT * FROM tbl_product where productName like '%$tensanpham%' ";
			$result = $this->db->select($query);
			return $result;
		}

		public function tbsao($id){
			$query = "SELECT id_product, round(AVG(sao),0) as slsao FROM `comment` where id_product='$id' GROUP BY id_product";
			$result = $this->db->select($query);
			return $result;
		}
		
		public function thongketheongay($date){
			$query = "SELECT *  FROM `tbl_order` WHERE date_order like '$date %'";
			$result = $this->db->select($query);
			return $result;
		}

		public function dabanhomnay($date){
			$query = "SELECT SUM(quantity) as daban  FROM `tbl_order` WHERE date_order like '$date %'";
			$result = $this->db->select($query);
			return $result;
		}


		public function tongtiensp($date){
			$query = "SELECT SUM(quantity*price) as daban  FROM `tbl_order` WHERE date_order like '$date %'";
			$result = $this->db->select($query);
			return $result;
		}

		public function product_brand($brand){
			$query = "SELECT * FROM `tbl_product` WHERE catid='$brand' limit 3";
			$result = $this->db->select($query);
			return $result;
		}
	}
